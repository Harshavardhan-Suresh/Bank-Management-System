...
public class SignUpWindow extends JFrame implements ActionListener
{
	...
	JTextField Aadhaar = new JTextField("",20);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(e.getActionCommand().equals("Create Account"))
		{
			...
			catch (InvalidMobileNumberException c) {
				JOptionPane.showMessageDialog(null,"Invalid mobile number","Failed", JOptionPane.ERROR_MESSAGE );
			}		
			catch (InvalidAadhaarException c) {
				JOptionPane.showMessageDialog(null,"Invalid aadhaar","Failed", JOptionPane.ERROR_MESSAGE );
			}	
		}
	}
	
	boolean isFormFilled() throws InvalidMobileNumberException, InvalidAadhaarException
	{			
		if (InvalidMobileNumberException.checkValidMobileNumber(phoneNo.getText()) == false) {
			throw new InvalidMobileNumberException();
		}
		if (InvalidAadhaarException.checkValidAadhaar(Aadhaar.getText()) == false) {
		...
		if(firstNameT.getText().isEmpty() || lastNameT.getText().isEmpty() 
				...
				|| Aadhaar.getText().isEmpty() || address.getText().isEmpty() || occupation.getText().isEmpty())
		{
			return false;
		}
	...
	void createNewAccount()
	{
		...
		UserInformation u= new UserInformation(firstNameT.getText(), lastNameT.getText(), email.getText(),
				phoneNo.getText(), Aadhaar.getText(), address.getText(), occupation.getText(), 
				sex);
